...
import JSZip from "https://cdn.skypack.dev/jszip@3.6.0/dist/jszip.min.js";
...
const loadBuilding = async (url, position, rotation, ) => {
  const response = await fetch(url);
  const arrayBuffer = await response.arrayBuffer();
  const zip = await JSZip.loadAsync(arrayBuffer);
  const glbFile = await zip.file(/.+\.glb$/i)[0].async("arrayBuffer");
  new GLTFLoader().load(glbFile, ({ scene: model }, animations) => {
...
new GLTFLoader().load("https://raw.githack.com/5Bianca/Blender/main/ground5.glb",
...
].forEach(([position, rotation]) => loadBuilding("https://raw.githack.com/5Bianca/Blender/main/Building4.glb.zip", position, rotation));
